...
/**
 * FIFA-style markings in metres: viewBox width = touchline (68m), height = length (105m).
 * Goals at y=0 (top) and y=105 (bottom). preserveAspectRatio + pitch-surface keeps circles/arcs true.
 */
const W = 68;
const L = 105;
const MID = L / 2;
...
/* Intersections of penalty arc (circle at spot, radius ARC_R) with penalty-area line */
function penaltyArcXs(goalY) {
...
const FieldSVG = () => (
  ...
  >
    ...
    <line x1="0" y1={MID} x2={W} y2={MID} className="fl" />

    <circle cx={CX} cy={MID} r={CR} className="fl" />
    <circle cx={CX} cy={MID} r="0.22" className="fl-dot" />

    {/* Top — penalty box, goal box, spot, arc (bulge toward midfield / +y) */}
    ...
    <path
      d={`M ${topArc.x1} ${PA_D} A ${ARC_R} ${ARC_R} 0 0 1 ${topArc.x2} ${PA_D}`}
      className="fl"
    />
    ...
    <path
      d={`M ${botArc.x1} ${L - PA_D} A ${ARC_R} ${ARC_R} 0 0 0 ${botArc.x2} ${L - PA_D}`}
      className="fl"
    />

    {/* Corner quadrants: arc centred on corner, radius 1m inside field */}
    <path d={`M ${CORNER_R} 0 A ${CORNER_R} ${CORNER_R} 0 0 1 0 ${CORNER_R}`} className="fl" />
    <path
      d={`M ${W - CORNER_R} 0 A ${CORNER_R} ${CORNER_R} 0 0 0 ${W} ${CORNER_R}`}
      className="fl"
    />
    <path
      d={`M ${CORNER_R} ${L} A ${CORNER_R} ${CORNER_R} 0 0 0 0 ${L - CORNER_R}`}
      className="fl"
    />
    <path
      d={`M ${W - CORNER_R} ${L} A ${CORNER_R} ${CORNER_R} 0 0 1 ${W} ${L - CORNER_R}`}
      className="fl"
    />
  </svg>
);

export default function Pitch({ lineup, onSlotClick, onSlotRemove }) {
